...
- **提交订正表单**:从某条目「订正」入口进入会自动带上报告对象,也可手动粘贴页面 URL;可选「问题类型」「涉及字段」,必填「正确内容应为」,选填来源 URL、补充说明、联系方式(仅审核者跟进、不公开)。
- **诚实的反馈**:提交后会明确告知"志愿者将审核(通常需几天),此时数据不会立即更改",各类错误都有清晰提示,绝不假装成功。
